...
beautiful.init("/home/adam/.config/awesome/theme.lua")
...
mymusic = awful.widget.watch(
	"ncmpcpp --current-song='%a - %t'",
	5,
	function(widget,out)
		if out then
			widget:set_text('  ' .. out:sub(1,40))
		else
			widget:set_text("")
		end
	end
...
mywifi = awful.widget.watch(
	...
	5,
	function(widget,out)
		second_line = split(out,"\n")[2]
		if second_line then 
			name = split(second_line," ")[1]
			widget:set_text("直 " .. name)
		else
			widget:set_text("睊 no WiFi")
		end
	end
)
...
mybattery = awful.widget.watch(
	...
	function(widget,out)
		lines = split(out,"\n")
		val = tonumber(lines[1])
		level = 10
		icon = ""
		icons={
			"","","","","","","","","",""
		}
		for i,ic in pairs(icons) do
			if val >= level then
				icon = ic
			end
			level = level + 10
		...
		if lines[2] == "Charging" then 
			icon = icon .. ""
		end

		widget:set_text(icon .. " " .. val .. "%")
	end
)
...
globalkeys = gears.table.join(
	...
	{description = "toggle panel bars", group = "awesome"})	
